...
-- Function to create TradFi OHLCV table for a specific symbol
CREATE OR REPLACE FUNCTION create_tradfi_ohlcv_table(symbol_name TEXT)
RETURNS VOID AS $$
DECLARE
    table_name TEXT;
BEGIN
    table_name := lower(symbol_name) || '_tradfi_ohlcv';
    ...
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I (
            ...
            -- Primary key on timestamp for this symbol
...
-- Function to create Crypto OHLCV table for a specific symbol
CREATE OR REPLACE FUNCTION create_crypto_ohlcv_table(symbol_name TEXT, exchange_name TEXT)
RETURNS VOID AS $$
DECLARE
    table_name TEXT;
    clean_symbol TEXT;
BEGIN
    ...
    table_name := clean_symbol || '_' || lower(exchange_name) || '_crypto_ohlcv';
    ...
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I (
            ...
            -- Primary key on timestamp for this symbol/exchange pair
            CONSTRAINT %I PRIMARY KEY (timestamp)
        )', table_name, table_name || '_pkey');
...
-- Function to get table name for symbol
CREATE OR REPLACE FUNCTION get_table_name(symbol_name TEXT, asset_type TEXT, exchange_name TEXT DEFAULT NULL)
RETURNS TEXT AS $$
DECLARE
    clean_symbol TEXT;
    table_name TEXT;
